...
function flip_Card()  { //flip card work with some conditions 
  if (!can_click) { // cant click on delay 
      return;
  }
  if (this === first_Card) { //if the user click the same card again 
      return;
  }
...
function reset_Board() { //reseting after checking 
...
function Match_check() {
    ...
    if (first_Card.getAttribute("name") === second_Card.getAttribute("name")) { //checking by name 
